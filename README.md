...
Desktopowa fabryka izometrycznych assetów dla Unity. Aplikacja łączy lokalny Codex App Server, skill `imagegen` i opcjonalne lokalne renderery ComfyUI oraz `stable-diffusion.cpp` z przenośnym registry SQLite, wersjonowanym review oraz eksportem PNG + JSON do katalogu `Assets`.
...
npm run package   # rozpakowana aplikacja Windows
npm run make      # instalator Squirrel i ZIP
```
...
1. Utwórz przenośny projekt i ustaw bazową szerokość kafla, brief oraz PPU. Wysokość kafla jest zawsze wyliczana z proporcji 2:1, więc dla szerokości 256 px wynosi 128 px. Kliknięcie nazwy projektu w górnym pasku otwiera stronę projektu, na której ustawienia można później edytować.
2. Podaj nazwę pojedynczego assetu, wybierz jego typ i opcjonalnie doprecyzuj go opisem. Sama nazwa wystarcza do uruchomienia generacji. Job trafia do trwałej kolejki i uruchamia `$imagegen` przez lokalny `codex app-server`. W ustawieniach projektu można wybrać od 1 do 8 jednoczesnych zadań; domyślny limit wynosi 1. Zadania różnych assetów mogą działać równolegle, natomiast kolejne wersje tego samego assetu zachowują kolejność. Można też wyłączyć weryfikację AI po generowaniu — Codex wtedy nie otwiera finalnego obrazu do oceny ani nie wykonuje automatycznej korekty. Gotowa wersja zachowuje stan nieweryfikowany i pokazuje w nagłówku akcję **Weryfikacja**, która pozwala później uruchomić samą ocenę istniejącego obrazu, bez ponownego generowania. Lokalne, deterministyczne kontrole formatu PNG, wymiarów, geometrii, szwów i połączeń pozostają aktywne.
   - `Flat tile` oznacza pełny, bezszwowy romb terenu i ma canvas równy bazowemu kaflowi projektu.
   - `Elevated tile` dodaje wysokość wyrażoną w poziomach. Poziom `N` daje canvas o wysokości `bazowa wysokość × (1 + N)`; dla kafla 256×128 px i poziomu 2 wynik ma 256×384 px.
   - `Road tile` jest kompletnym zestawem transparentnych nakładek 1×1 tworzonym w jednym jobie. Imagegen przygotowuje tylko jedną nieprzezroczystą, pełnokadrową próbkę materiału nawierzchni. Aplikacja buduje z niej deterministycznie wszystkie 16 geometrii, nakłada alfę i identyczne porty połączeń oraz tworzy izolowany fragment, cztery zakończenia, dwie proste, cztery zakręty, cztery warianty T i skrzyżowanie. Każdy kafel jest walidowany osobno, a review pokazuje cały zestaw jako siatkę 4×4. Zielone/różowe chroma-key i źródła z przezroczystym tłem są odrzucane przed utworzeniem zestawu.
   - `Building` ma relatywną szerokość i wysokość względem bazowego kafla. Domyślne `1×2` przy bazie 256×128 px daje canvas 256×256 px.
   - `Character` działa tak samo, z domyślnym rozmiarem `0,5×1,5`, czyli 128×192 px przy tej samej bazie.
...
6. Wyeksportuj bieżące zatwierdzone wersje do `Assets/TilemapGenerator/<kategoria>/`. Zestaw drogi trafia do własnego katalogu jako `road-00.png`…`road-15.png`, a manifest zawiera maskę i kierunki każdego pliku. Unity tworzy z nich jeden `RoadRuleTile.asset`, który dobiera wariant automatycznie podczas malowania. Każdy zatwierdzony `flat_tile` i `elevated_tile` dostaje też atlas blendingu oraz gotowe assety auto-tile w Unity.

## Terrain blending i auto-tile w Unity

Eksport terenu nie generuje kombinacji parami. Dla każdego zatwierdzonego terenu aplikacja tworzy jeden atlas `--blend.png` w układzie 8×6 z kanonicznymi 47 maskami blob oraz osobny `--walls.png`. Koszt rośnie liniowo z liczbą terenów. Atlas zawiera tylko górną płaszczyznę, a plik ścian tylko fragment poniżej rombu.
...
- prefab `Assets/TilemapGenerator/Generated/Prefabs/TerrainGrid.prefab` z siatką izometryczną, bazą, warstwami blend i ścianami.

Dla każdej drogi Unity importuje 16 wariantów jako sprite'y i tworzy `Assets/TilemapGenerator/Generated/Roads/<asset-id>/RoadRuleTile.asset`. Reguły sprawdzają czterech sąsiadów izometrycznej komórki (NW, NE, SE i SW), więc pojedynczy asset obsługuje odcinki, zakręty, zakończenia, wszystkie cztery warianty T oraz skrzyżowanie. Prefab `TerrainGrid.prefab` dostaje osobną Tilemapę `Roads`; przeciągnij `RoadRuleTile.asset` do Tile Palette i maluj nim na tej warstwie.
...
Ze względów bezpieczeństwa akceptowane są wyłącznie adresy loopback (`127.0.0.1`, `localhost` i `::1`). Pierwszy zarządzany profil to **Z-Image Turbo**. Współpracuje z bieżącym szablonem Comfy Desktop i wymaga:
...
Trzeci renderer uruchamia bezpośrednio `sd-cli.exe`, bez serwera ComfyUI. Na stronie projektu znajduje się zarządzany instalator, który:
...
Instalacja zaczyna się dopiero po kliknięciu przycisku. Zarządzane pliki trafiają do `%LOCALAPPDATA%\Tilemap Generator\stable-diffusion.cpp`. Dostępne profile:
...
Każdy projekt jest samodzielnym katalogiem:
...
Główny proces zapisuje log JSONL w `%AppData%\Tilemap Generator\logs\main.jsonl`. Ścieżka jest również widoczna na ekranie Diagnostyka. Log obejmuje błędy turnów, stderr i awarie protokołu App Servera oraz identyfikatory joba, assetu i wersji — bez zapisywania promptów.
